...
function specifierCandidates(
  ...
  if (resolved === undefined) return undefined; // external package - not ours
  return [
    resolved,
    `${resolved}.ts`,
    `${resolved}.tsx`,
 ...
 */
const EMIT_EXTENSIONS: Record<string, readonly string[]> = {
  '.js': ['.ts', '.tsx'],
  '.jsx': ['.tsx'],
  '.mjs': ['.mts'],
  '.cjs': ['.cts'],
...
function sourceOfEmit(resolved: string): string[] {
  for (const [emit, sources] of Object.entries(EMIT_EXTENSIONS)) {
    if (!resolved.endsWith(emit)) continue;
...
function importSpecifiers(relPath: string, source: string): string[] {
  ...
  const walk = (node: ts.Node): void => {
    ...
    if (ts.isCallExpression(node) && node.arguments.length === 1) {
      ...
      if ((isDynamicImport || isRequire) && ts.isStringLiteral(arg)) specs.push(arg.text);
    }
...
function orderAlongEdges(
  ...
  for (const member of [...component].sort()) {
    if (!visited.has(member)) ordered.push(member);
  }
  return ordered;
}
...
export function circularDependencies(graph: ReadonlyMap<string, readonly string[]>): string[][] {
  ...
  const strongConnect = (v: string): void => {
    ...
    if (low.get(v) === index.get(v)) {
      ...
      if (component.length > 1) cycles.push(orderAlongEdges(component, graph));
    }
  };
