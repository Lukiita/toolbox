...
function isToolboxConfig(value: unknown): value is ToolboxConfig {
 ...
 */
export async function loadToolboxConfigPartial(root: string): Promise<ToolboxConfig> {
  ...
  if (!isToolboxConfig(loaded.default)) {
    ...
    throw new Error(
      `${file}: expected a default export object ({ quality?, hooks? }), received ${received}`,
    );
  }
 ...
 */
export async function loadToolboxConfig(root: string): Promise<ResolvedToolboxConfig> {
