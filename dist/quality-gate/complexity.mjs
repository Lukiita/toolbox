 ...
 */
function isFieldInitializer(node) {
...
function functionName(node) {
    ...
    if (ts.isPropertyDeclaration(node))
        return `${node.name.getText()} (field)`;
    const named = node;
    if (named.name)
...
function descend(node, walk) {
    ts.forEachChild(node, (child) => {
        if (isFunctionLike(child))
            return; // the nested function scores on its own
        walk(child);
    });
}
/** Every function in one source file, with its own score. Pure. */
export function functionComplexities(relPath, source) {
...
export function overComplexFunctions(all, limit = DEFAULT_QUALITY.ccLimit) {
    return all
        .filter((f) => f.cc > limit)
