...
import { isSizedFile } from "./size.mjs";
/**
...
export function uncoveredLines(entry) {
...
export function lineTotals(entry) {
    const { instrumented, covered } = classifyLines(entry);
 ...
 */
export function isTestableFile(relPath, sourceWindow = DEFAULT_QUALITY.sourceWindow) {
 ...
 */
export function coveragePercent(map, { toRelativePath, sourceWindow = DEFAULT_QUALITY.sourceWindow }) {
    let total = 0;
    let covered = 0;
    for (const [absolute, entry] of Object.entries(map)) {
        if (!isTestableFile(toRelativePath(absolute), sourceWindow))
            continue;
 ...
 */
export function uncoveredInTestableFiles(map, { toRelativePath, sourceWindow = DEFAULT_QUALITY.sourceWindow }) {
    const out = [];
    for (const [absolute, entry] of Object.entries(map)) {
        const file = toRelativePath(absolute);
        if (!isTestableFile(file, sourceWindow))
            continue;
