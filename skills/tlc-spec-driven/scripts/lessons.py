...
def _norm(text):
    ...
    t = text.lower().strip()
    # Fold diacritics instead of deleting them. An ASCII-only class would turn
    # "ação" into "a o" — losing letters, colliding unrelated words, and never
    # merging with "acao". Folding makes the three spellings one key.
...
def cmd_penalize(root, args):
    ...
    if not target:
        ...
        return 2
    # Only confirmed lessons are ever loaded as guidance, so only they can fail
    # when applied. Penalizing a candidate would quarantine advice that was
    # never trusted in the first place.
    if target["status"] != "confirmed":
        print(
            f"ERROR: lesson {target['id']} is {target['status']}, not confirmed — "
            "only confirmed lessons can be penalized",
            file=sys.stderr,
        )
        return 2
