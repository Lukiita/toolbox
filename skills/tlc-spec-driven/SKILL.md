...
**Script paths.** Shell commands in this skill and its references (`lessons.py`, `capabilities.py`) are written for the canonical global install: `~/.agents/skills/<skill>/scripts/…`. If this skill is running from a different location (e.g. a project-local `.agents/skills/` copy), swap that prefix for the skill directory your runtime actually reported — same script, same arguments, only the prefix moves.
...
- **Discuss is triggered within Specify** when the agent detects ambiguous gray areas that need user input — including any implicit-requirement dimension (persistence/state, external calls, auth, payments, concurrency, state transitions) the feature touches and leaves unresolved. A dimension already settled by the spec, a project convention, or a recorded decision is not a gray area
...
├── capabilities/       # Living behavioral contract, one file per domain area
│                       # (owned by the capability-sync skill; synced at validation Step 11)
...
- confirmed lessons — load at Specify and Design via `python3 ~/.agents/skills/tlc-spec-driven/scripts/lessons.py list --status confirmed` ([lessons.md](references/lessons.md)); confirmed only, never candidates
...
Step 2: Project docs → README, docs/, inline comments, `CONTEXT.md` (canonical vocabulary — specs and designs use its terms), `docs/adr/` (accepted architectural decisions), `.specs/STATE.md` (Decisions)
