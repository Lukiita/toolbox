...
**Skipping the phase never skips step 1.5.** The **Test Coverage Matrix** and **Gate Check Commands** are what Execute and Validate read to know which tests a task owes and which command decides "done" — without them the gate is undefined, and a small feature is exactly where that goes unnoticed. When you skip the phase, still run step 1.5 and render both sections into the inline execution plan, under those exact headings. Everything downstream that says "from tasks.md" means "from tasks.md, or from the inline plan when the phase was skipped".
...
| Code Layer | Required Test Type | Coverage Expectation | Location Pattern | Run Command |
| ---------- | ------------------ | -------------------- | ---------------- | ----------- |
| Service | unit | All branches; 1:1 to spec ACs; all listed edge cases | `src/**/__test__/*.spec.ts` | `pnpm test:unit` |
| Repository | integration | Key query paths + error paths | `src/**/__test__/*.e2e-spec.ts` | `pnpm test:e2e` |
| Controller/Resolver | e2e | All routes: happy + edge + error | `src/**/__test__/*.e2e-spec.ts` | `pnpm test:e2e` |
