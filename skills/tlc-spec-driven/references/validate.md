...
2. **Feature-level validation (independent Verifier sub-agent, always-on, never prompted):** After all tasks for a feature (or priority group) are done, validation runs automatically — the orchestrator dispatches a **fresh Verifier sub-agent** (see [sub-agents.md](sub-agents.md)). Do NOT ask the user whether to run it; it is the safety net, not an opt-in. User interaction is limited to interactive UAT (for user-facing features) and acting on a FAIL verdict ("fix these gaps now?"). The Verifier:
   ...
   - Runs the **spec-anchored outcome check**, the **capability regression check** and the **discrimination sensor** (all described below)
...
### 2b. Capability Regression Check (`.specs/capabilities/`)

The spec-anchored check verifies the feature against its own frozen spec. This step anchors the other direction: the feature must not have broken a **durable capability** — the living behavioral contract in `.specs/capabilities/`, owned by the `capability-sync` skill. This is the regression net between features; without it, only the newest spec is ever verified.

1. List the contract: `python3 ~/.agents/skills/capability-sync/scripts/capabilities.py list`. If `.specs/capabilities/` does not exist yet, mark the check as `— (no capabilities defined)` and move on.
2. Identify the capabilities whose scope overlaps the feature's git diff surface.
3. For each overlapping requirement/scenario, confirm the guaranteed behavior still holds: trace it to current code/tests (`file:line`) or exercise it. Evidence-or-zero applies here too — an overlapping scenario with no evidence counts as NOT verified, never as an implicit pass.
4. Record results in the report (Capability Regression section below). A broken capability scenario is a ranked gap with the same severity rules as a failed AC.
...
Run the Build-level gate check from the **Gate Check Commands** section in tasks.md — or, when the Tasks phase was skipped, from the same section in the Execute inline plan ([implement.md](implement.md) step 0). This is NOT optional. If neither carries the section, derive it now via [tasks.md](tasks.md) step 1.5 and record in the report that it was missing — a feature that reached validation without a defined gate is itself a finding.

1. Run: `[Build gate command from the Gate Check Commands section]`
...
2. **Record the exact code commit you verified** in the `**Commit verificado**` field — `git log -1 --format=%H -- . ':!.specs'`, the full SHA of the last commit that touched anything outside `.specs/`. The report claims "this code is verified"; without the SHA, *which* code is a guess. It is what makes "did anything change after the stamp?" a decidable question instead of a judgement call.

   **Use that expression, not `git rev-parse HEAD`.** The stamp must survive the commits the flow itself makes *after* verifying: this very report is committed after it is written, and a capability sync usually commits on top. A raw HEAD stamp is therefore stale the instant it is written — the comparison would report drift on every run, which is the same as reporting nothing. Excluding `.specs/` cuts exactly the class of commits the flow itself makes after stamping, so the field means **the last commit outside `.specs/`**: evidence landing later leaves it untouched, a source change moves it.
...
This is a closing action of validation — not a separate phase. Immediately after the report is written, turn its grounded failures into reusable, project-local guidance by following [lessons.md](lessons.md). In short: for each surviving mutant, spec-precision gap, failed/uncovered AC, or `// SPEC_DEVIATION`, record one terse general lesson via `python3 ~/.agents/skills/tlc-spec-driven/scripts/lessons.py add` (the script enforces grounding and owns all bookkeeping). A clean PASS with no signal → record nothing. Run the self-check: if there was signal but no lesson was recorded, say so in chat. See [lessons.md](lessons.md) for the exact commands, phrasing rules, scope discipline, and the no-script fallback.

### 11. Sync Capabilities (MANDATORY on PASS)

Closure must not depend on anyone remembering to ask for it. When the verdict is PASS (including a PASS reached after the fix→re-verify cycle), invoke the **`capability-sync`** skill: fold the feature's verified requirements — per the evidence table in `validation.md` — into `.specs/capabilities/`, then run its mechanical gate:

```bash
python3 ~/.agents/skills/capability-sync/scripts/capabilities.py check   # must exit 0
```

If the Skill tool is unavailable (headless run), follow `~/.agents/skills/capability-sync/SKILL.md` directly — the process is the same. A FAIL verdict syncs nothing: unverified behavior never enters the contract.
...
**Capability check**: [N overlapping scenarios verified, 0 broken | — no capabilities defined]
...
| Capability | Requirement / Scenario | Evidence (`file:line` or exercised flow) | Result |
| ---------- | ---------------------- | ---------------------------------------- | ------ |
| project-b-schedule | Bloqueio remove disponibilidade | `src/lib/availability.test.ts:88` — assertion still targets the guaranteed outcome | ✅ Holds |

**Status**: ✅ No overlapping capability broken / ❌ Regression found / — (no `.specs/capabilities/` yet)
...
- **Distill after writing** — turn grounded failures into lessons via `~/.agents/skills/tlc-spec-driven/scripts/lessons.py` ([lessons.md](lessons.md)); clean PASS → no lesson
- **Sync on PASS** — validation is not closed until `.specs/capabilities/` reflects the verified behavior (Step 11, `capability-sync` skill)
