...
**Where the substance lives** (see the `domain-modeling` skill): domain vocabulary never becomes an AD entry - if the decision is really "what does this word mean", resolve it in the `CONTEXT.md` glossary instead. And in repos that keep an ADR directory (`docs/adr/` or the repo's existing one), a decision that clears the three-part test is written as an ADR there, and the `AD-NNN` entry becomes a one-line pointer (`- **Decision**: see docs/adr/NNNN-slug.md`) - the AD log keeps the sequence and the audit trail, the ADR keeps the substance, nothing is recorded twice. Without an ADR directory, the full `AD-NNN` entry is the record.
