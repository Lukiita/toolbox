...
**Two escalations keep this log talking to the repo's other memories** (see the `domain-modeling` skill):

- **Domain vocabulary never becomes an AD entry.** If the decision is really "what does this word mean", resolve it in the `CONTEXT.md` glossary instead.
- **Architectural decisions get promoted to an ADR.** When a decision is hard to reverse, surprising without context, AND the result of a real trade-off (all three), write it as an ADR in `docs/adr/` (format: `domain-modeling` skill) and still append the `AD-NNN` entry — but as a single pointer line (`- **Decision**: see docs/adr/NNNN-slug.md`). The AD log keeps the sequence and the audit trail; the ADR keeps the substance; nothing is recorded twice.
