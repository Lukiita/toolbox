...
**Speak the project's language:** if the repo has a `CONTEXT.md` glossary (see the `domain-modeling` skill), load it during the scan and write the spec in its canonical terms — never a synonym the glossary marks as _Avoid_. A feature that surfaces a new term, or a conflict with an existing one, resolves it through `domain-modeling` (update the glossary) before the spec freezes; a spec written in off-glossary vocabulary bakes the ambiguity into every downstream phase.
