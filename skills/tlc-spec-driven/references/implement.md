...
The plan MUST also carry the **Test Coverage Matrix** and **Gate Check Commands** sections, derived exactly as in [tasks.md](tasks.md) step 1.5 - they are what the gate steps below and the Verifier read. Skipping the Tasks phase never skips them: with no matrix there is no "which tests does this owe", and with no gate commands "the gate must pass" has nothing to run. From here on, every "from tasks.md" means "from tasks.md, or from this inline plan".
