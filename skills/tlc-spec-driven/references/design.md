...
**Also honor the repo's durable records:** accepted ADRs in `docs/adr/` (or the repo's existing ADR directory) bind this design exactly like active `AD-NNN` entries — conform, or supersede explicitly with a new ADR. And anchor the design's vocabulary to `CONTEXT.md` when it exists: components, entities, and events carry glossary names, not synonyms.

**Also load confirmed lessons** relevant to this feature: `python3 ~/.agents/skills/tlc-spec-driven/scripts/lessons.py list --status confirmed` (filter with `--scope`/`--query`). These are past verification failures distilled into guidance — apply them while designing. Load only `confirmed`. Skip silently if no store or no code tool. See [lessons.md](lessons.md).
...
### 6. List Files Touched

After defining components, list every file the design expects to create or change in `## Files Touched`. This is what makes the design reviewable at a glance — `Tech Decisions` and `Risks & Concerns` explain *why* and *what could go wrong*, but neither answers "how big is this?". Without this list, that answer only emerges by collecting each component's `Location:` line one at a time.
...
| File | Change |
| ---- | ------ |
| `src/path/to/file.ts` | New / Modified / Deleted — one line on what changes |
...
- **List Files Touched** — the size of the change should be readable in one table, not reconstructed from scattered `Location:` lines
