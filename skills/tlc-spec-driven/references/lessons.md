...
| `~/.agents/skills/tlc-spec-driven/scripts/lessons.py` | package | The only way to mutate lessons. |
...
```bash
python3 ~/.agents/skills/tlc-spec-driven/scripts/lessons.py penalize --id L-NNN
```

Two penalties quarantine it. Use sparingly and only on real repeats. The script only accepts `confirmed` lessons here — candidates were never loaded as guidance, so they cannot have failed when applied.
...
python3 ~/.agents/skills/tlc-spec-driven/scripts/lessons.py list --status confirmed

# Or filter by the area this feature touches:
python3 ~/.agents/skills/tlc-spec-driven/scripts/lessons.py list --status confirmed --scope billing
python3 ~/.agents/skills/tlc-spec-driven/scripts/lessons.py list --status confirmed --query "idempotency"
```
